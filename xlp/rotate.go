...
func Rotate(path string, maxsize string, compress bool) io.WriteCloser {
	...
	if err := f.open(); err != nil {
		log.Printf("[WARN] create rotate file: %v", err)
...
func (f *rotateFile) setMaxsize(s string) {
	if s = strings.TrimSpace(s); s != "" {
		...
		if n, err = strconv.ParseFloat(s, 64); err != nil {
			log.Printf("[WARN] %q is not a valid filesize", s)
			return
		}
		...
		// log.Printf("logfile %s maxsize: %s -- %f -- %d -- %s", f.path, s, u, f.maxsize, formatSize(f.maxsize))
	}
}

...
`, SYNOPKG_PKGVER)
	...
	logrus.Infof("Config")
	logrus.Infof("  - Port:             %s", Iif(d.DashboardPort == 0, "random", strconv.Itoa(d.DashboardPort)))
	logrus.Infof("  - Host:             %s", d.DashboardHost)
	logrus.Infof("  - User:             %s", Iif(d.DashboardUsername == "", "none", d.DashboardUsername))
	logrus.Infof("  - Pass:             %s", Iif(d.DashboardPassword == "", "none", d.DashboardPassword))
	logrus.Infof("  - DownloadPath:     %s", d.DirDownload)
	logrus.Infof("  - DataPath:         %s", d.DirData)
	logrus.Infof("  - Logger.Target:    %s", d.Logger.Target)
	logrus.Infof("  - Logger.Maxsize:   %s", d.Logger.Maxsize)
	logrus.Infof("  - Logger.Compress:  %s", Iif(d.Logger.Compress, "√", "×"))

	if err := d.run(ctx); err != nil {
		logrus.Infof("exited: %v", err)
	} else {
		logrus.Infof("exited!")
	}
}
...
func (d *Daemon) run(ctx context.Context) (err error) {
	...
	logrus.Infof("Environ")
	for _, envIt := range environs {
		logrus.Infof("  - %s", envIt)
	}
...
func (d *Daemon) mockSyno(ctx context.Context, environs []string) (err error) {
	...
	go func() {
		...
		logrus.Infof("xlp starting, listen at %v", s.Addr)

		if err := s.ListenAndServe(); err != nil {
			if err != http.ErrServerClosed {
				logrus.Infof("xlp done: %v", err)
				return
			}
		}

		logrus.Infof("xlp is done")
	}()
...
func fileWrite[T ~string | ~[]byte](path string, data T, perm os.FileMode) (err error) {
	...
	if f, err = os.OpenFile(path, os.O_WRONLY|os.O_CREATE|os.O_EXCL, 0666); err != nil {
		if os.IsExist(err) {
			logrus.Warnf("%s has exist, ignore write", path)
			return nil
		}
		return
	}
